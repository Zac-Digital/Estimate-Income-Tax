...
test.describe('Pension Contribution Fixed Page', () => {
    test.beforeEach(async ({page}) => {
        await page.goto('/pension-contribution-fixed');
    });

    test('has title', async ({page}) => {
        await expect(page).toHaveTitle('Pension Contribution Fixed - GOV.UK');
    });
    ...
    test('swap link navigates to alternative pension contribution and clears input', async ({page}) => {
        ...
        expect(page.url()).toContain('/pension-contribution-percentage');
